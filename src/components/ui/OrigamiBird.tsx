...
export function OrigamiBird() {
  ...
  const storks = useMemo(() => {
    const all = [
      { top: '15%', delay: 0, duration: 25, size: 120, opacity: 0.9 },
      { top: '35%', delay: 8, duration: 35, size: 90, opacity: 0.7 },
      { top: '60%', delay: 4, duration: 55, size: 75, opacity: 0.8 },
    ]
    return isMobile ? [all[0]] : all
  }, [isMobile])

  const wingDuration = isMobile ? 1.2 : 0.5

  return (
    <div className="pointer-events-none fixed inset-0 z-10 overflow-hidden">
      {storks.map((stork, i) => (
        <StorkInstance key={`stork-${i}`} {...stork} wingDuration={wingDuration} />
      ))}
    </div>
  )
}

function StorkInstance({ top, delay, duration, size, opacity = 1, wingDuration }: {
  top: string; delay: number; duration: number; size: number; opacity?: number; wingDuration: number
}) {
  return (
    <motion.div
      className="absolute left-0"
      style={{ top, width: size, opacity, willChange: 'transform' }}
      initial={{ x: '-150%' }}
      animate={{ x: '120vw' }}
